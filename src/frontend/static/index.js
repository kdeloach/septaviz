...
function getMarkerClassName(loc) {
  var result = ["vehicle-marker-inner", this.getMarkerDirection(loc)];
...
function addRoute(routeNum) {
  ...
  return fetchRouteTrace(routeNum).then(addRouteTrace);
}

function addRouteThenFitBounds(routeNum) {
  addRoute(routeNum).then(fitBounds);
}
...
function fetchRouteTrace(routeNum) {
  return fetch("static/stops/" + routeNum + ".geojson", { cache: true }).then(
    function (geojson) {
...
function fetchVehicles(routeNum) {
  ...
  return fetch(url, { dataType: "jsonp", cache: false }).then(function (data) {
